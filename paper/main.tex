...
\begin{table}[t]
% Data: hmax_control_results_n10.json (hmax_control_experiment.py, budget 600, seeds 0-9);
% supersedes the 3-seed hmax_control_results.json from the same experiment.
\caption{$\hmax$ control (600 evals, 10 seeds).}
...
\begin{table}[t]
\centering
\caption{CIFAR-10/CNN full-batch replication: final training loss (mean$\pm$std over seeds $\{0,1,2\}$), test accuracy, and controller saturation at a 600-evaluation budget. \emph{Placeholders pending shard completion.}}
\label{tab:cifar}
\vskip 0.1in
\small
\begin{tabular}{lccc}
\toprule
Config & Final loss & Test acc (\%) & Steps at $h_{\max}$ (\%) \\
\midrule
...
\textbf{Results.} \emph{[Pending: verdicts on claims (i)--(iii) with Table~\ref{tab:cifar}; frac-at-$h_{\max}$ and bit-identity check across rtol; note any divergence from the MNIST picture.]}
